...
BYTE ToneMasks[13][14]  = {
{0x17, 0x24, 0x0,  0xFF, 0xFF, 0xFF, 0x0F, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, // 0 =  Cenelec A36
...
//	SEGGER_RTT_printf(0,"SNR:%2d.%02ddB\r\n",GetSnr(_pDataTransferIndication->_LQI)/100,
//										   GetSnr(_pDataTransferIndication->_LQI)%100);
				...
				*moreLen = (plcDataLen-pos) - _header->_Length - sizeof(struct Header);
				if (_header->_Id == Data_Transfer_Id)
				{
					if (_header->_Length != 8)
					{
						...
						//printf("recv the data:%s\n", buf);	
					SNR_INT=GetSnr(_pDataTransferIndication->_LQI)/100;
					SNR_DOT=GetSnr(_pDataTransferIndication->_LQI)%100;	
         	SEGGER_RTT_printf(0,"SNR:%2d.%02ddB\r\n",SNR_INT,SNR_DOT);			
